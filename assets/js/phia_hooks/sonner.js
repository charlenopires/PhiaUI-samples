...
const VARIANT_CLASSES = {
  ...
  success: "bg-background text-foreground border-border [&_[data-sonner-icon]]:text-green-500",
  error: "bg-background text-foreground border-border [&_[data-sonner-icon]]:text-red-500",
  warning: "bg-background text-foreground border-border [&_[data-sonner-icon]]:text-yellow-500",
  info: "bg-background text-foreground border-border [&_[data-sonner-icon]]:text-blue-500",
};
...
function entryTranslate(position) {
  return position.startsWith("top-") ? "-translate-y-2" : "translate-y-2";
}
...
const PhiaSonner = {
  ...
  _createToast({ title, description, variant = "default", duration_ms = 4000 }) {
    ...
    if (existing.length >= this.maxVisible) this._dismiss(existing[0]);
    ...
    node.innerHTML = `
      ...
      <button type="button" aria-label="Close" data-sonner-close
    ...
    if (duration_ms > 0) node._dismissTimer = setTimeout(() => this._dismiss(node), duration_ms);
  },
  _dismiss(node) {
    if (!node || !node.isConnected) return;
    if (node._dismissTimer) clearTimeout(node._dismissTimer);
    const exit = entryTranslate(this.position);
    node.classList.remove("opacity-100");
    node.classList.add("opacity-0", exit);
    const cleanup = () => { if (node.isConnected) { node.remove(); this._restack(); } };
